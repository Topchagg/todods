...
const DeskForm = () => {
  const { register, handleSubmit, formState: { errors } } = useForm<DeskFormData>();
  const { postData, loading, error, success } = usePostFirestore<deskPostRequest>();
  ...
  return (
    <div>
      <form onSubmit={handleSubmit(onSubmit)} className="max-w-md mx-auto mt-8">
        ...
        {success && <p className="text-green-500 mt-4">Desk created successfully!</p>}
      </form>
    </div>
  );
};
